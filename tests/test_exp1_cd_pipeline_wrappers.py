"""Tests for Exp.1.c/d adapters in Luis's shared ml_pipeline."""
...
from src.utils.config import BiLSTMConfig, CNNConfig, ModelConfig, TransformerConfig


def _tiny_transformer_config(name: str, with_bilstm: bool = False) -> ModelConfig:
    return ModelConfig(
        name=name,
        patch_embed_dim=4,
        cnn=CNNConfig(
...
def _windowed_arrays(n_windows: int = 8, input_len: int = 6, n_features: int = 3, horizon: int = 2):
...
class Exp1CDPipelineWrapperTests(unittest.TestCase):
    def test_wrappers_inherit_from_correct_base_model(self) -> None:
        transformer = CNNTransformerPipelineModel(
            model_config=_tiny_transformer_config("cnn_transformer"),
            device="cpu",
        )
        bilstm_transformer = CNNBiLSTMTransformerPipelineModel(
            model_config=_tiny_transformer_config("cnn_bilstm_transformer", with_bilstm=True),
            device="cpu",
        )

        self.assertIsInstance(transformer, BaseModel)
        self.assertIsInstance(bilstm_transformer, BaseModel)
    ...
    def test_cnn_transformer_fit_predict_tiny_data(self) -> None:
        X, y = _windowed_arrays()
        model = CNNTransformerPipelineModel(
            model_config=_tiny_transformer_config("cnn_transformer"),
            device="cpu",
        )
    ...
    def test_cnn_bilstm_transformer_fit_predict_tiny_data(self) -> None:
        X, y = _windowed_arrays()
        model = CNNBiLSTMTransformerPipelineModel(
            model_config=_tiny_transformer_config("cnn_bilstm_transformer", with_bilstm=True),
            device="cpu",
        )
    ...
    def test_experiment_runner_runs_wrapper_on_synthetic_data(self) -> None:
        ...
        self.assertEqual(result["model_config"]["type"], "exp1d_cnn_transformer")
